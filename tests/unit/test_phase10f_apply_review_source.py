...
def test_race_a_editor_commits_before_apply_commit_rejects_apply(apply_env):
    """
    Race A: Apply captures V3 snapshot, but an editor commits V4 before Apply commits.
    Apply MUST reject as stale, and V4 MUST remain active.
    """
    uow_factory = apply_env["uow_factory"]
    storage = apply_env["storage"]
    doc_proc = apply_env["doc_proc"]
    ...
    # Simulate Editor committing V4 right after Apply captures V3
    original_store = storage.store

    def side_effect_store(*args, **kwargs):
        res = original_store(*args, **kwargs)
        if kwargs.get("filename", "").startswith("output_"):
            with uow_factory.create() as uow:
                j = uow.jobs.get_by_id(job.id)
                j.output_path = "file:///artifacts/output_1_v4.md"
                j.output_artifact_version_watermark = 4
                uow.jobs.save(j)
                uow.commit()
        return res
    ...
    with pytest.raises(StaleDocumentVersionError) as exc_info:
        service.apply_reviews(job_id=job.id)
    ...
    with uow_factory.create() as uow:
        ...
        assert j_final.output_path == "file:///artifacts/output_1_v4.md"
