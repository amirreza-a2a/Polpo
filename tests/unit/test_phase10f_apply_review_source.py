...
def test_race_a_editor_commits_before_apply_reservation_rejects_apply(apply_env):
    ...
    with original_create() as uow:
        ...
        assert j_final.output_artifact_version_watermark == 4
...
def test_race_b_editor_commits_during_apply_staging_rejects_apply(apply_env):
    ...
    assert editor_committed_ver == 5
    assert exc_info.value.base_version == 3
    assert exc_info.value.current_version == 5

    # Verify Editor's V5 remains the active output path in SQLite
    with uow_factory.create() as uow:
        j_final = uow.jobs.get_by_id(job.id)
        assert "output_1_v5.md" in j_final.output_path
        assert j_final.output_artifact_version_watermark == 5

    # Verify active text is the user's edits, NOT Apply's stale V4
    v5_handle = ArtifactHandle(
    ...
    v5_content = storage.retrieve(v5_handle).decode("utf-8")
    assert "# Editor V5 Authoritative User Edit" in v5_content
