...
**What to build:** Create a dedicated secondary GitHub Actions workflow (`.github/workflows/periodic-validation.yml`) to execute Tier 2 platform verification without impacting the sub-2-minute everyday PR feedback loop. This workflow executes: (1) native macOS Apple Silicon testing on the explicit `macos-14` (ARM64) runner with Python 3.12, and (2) Python 3.10 and 3.11 backward-compatibility testing on the reference Linux runner (`ubuntu-24.04`). The workflow provides three distinct execution triggers: scheduled weekly runs, post-tag verification on `v*` pushes, and a manual `workflow_dispatch` mechanism designed specifically for pre-release validation *before* publishing official release tags.
...
- Job 1: `test-macos` (`macos-14`, Apple Silicon ARM64, Python 3.12):
  ...
  - Executes `pytest -q` across all 922+ tests under native Darwin kernel, APFS case-preserving filesystem, and macOS PySide6 Cocoa bindings.
...
- Claiming verification of older macOS versions (macOS 12 and 13 are deprecated/unavailable on GitHub-hosted runners) or Intel x86_64 Mac hardware.
...
- [ ] Runner label for macOS is explicitly pinned to `macos-14` (Apple Silicon ARM64; avoiding floating `-latest`).
- [ ] `test-macos` executes all 922+ tests in headless offscreen mode without failure.
...
- *Support Disclaimers*: Documentation acknowledges that `macos-14` verifies modern Apple Silicon Macs; older macOS 12/13 versions and legacy Intel hardware are Tier 3 (best-effort) and cannot be tested via standard GitHub-hosted runners.
