...
- Configure Branch Protection on `master` using authenticated `gh api`:
  - Target: `PUT /repos/amirreza-a2a/Polpo/branches/master/protection`
  - Required Status Checks (`required_status_checks`):
    - `strict: true` (Enforces "Require branches to be up to date before merging" — prevents semantic merge races by requiring PR branches to be rebased/updated to the latest `master` tip before merging).
  ...
  - Linear History (`required_linear_history: true`):
    - Prevents merge commits on `master`, ensuring clean rebase or squash-and-merge history.
...
- Implementation Automation:
  ...
  - Verify with `gh api repos/amirreza-a2a/Polpo/branches/master/protection` that HTTP 200 is returned and all 4 contexts are enforced.
- Documentation:
...
- [ ] `scripts/configure_branch_protection.sh` exists, is executable, and contains the idempotent `gh api` payload for `master`.
- [ ] Branch protection is actively configured on `amirreza-a2a/Polpo` for the `master` branch.
- [ ] Querying `gh api repos/amirreza-a2a/Polpo/branches/master/protection` returns HTTP 200 with:
  - `strict: true`
...
- Run `gh api repos/amirreza-a2a/Polpo/branches/master/protection --jq '{strict: .required_status_checks.strict, contexts: .required_status_checks.contexts, enforce_admins: .enforce_admins.enabled}'` and assert matching output.
- Check `git status` to ensure only intended files were created.

## Failure & Rollback Considerations

- If an emergency hotfix requires direct push access during an operational outage, protection can be temporarily adjusted using `gh api -X DELETE /repos/amirreza-a2a/Polpo/branches/master/protection` by a repository administrator, then immediately reapplied via `scripts/configure_branch_protection.sh`.
